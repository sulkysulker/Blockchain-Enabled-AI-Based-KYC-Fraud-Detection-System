...
function normalizeProposal(raw, id, type) {
  ...
  return {
    ...
    type,
    proposedTarget:
      type === 'issuer' ? get('proposedIssuer', 0) : get('proposedVerifier', 0),
...
function IssuerDashboard({ contract, onTxStatus, walletProps, txStatus }) {
  ...
  async function handleReportFraud() {
    ...
    if (score < 0 || score > 100)
      throw new Error('Fraud score must be between 0 and 100.');
    ...
    try {
      ...
      throw new Error(
        `Cannot report fraud: KYC record not found for this identifier. ${error?.message || ''}`
      );
    }

    const receipt = await sendTx(
      contract.reportFraud(kycHash, score, fraudReason),
      onTxStatus
    );
  ...
  async function handleCreateProposal() {
    ...
    const txPromise =
      proposalType === 'issuer'
        ? contract.proposeIssuer(
            target,
            organizationName,
            justification,
            evidenceURI
          )
        : contract.proposeVerifier(
            target,
            organizationName,
            justification,
            evidenceURI
  ...
  async function fetchProposalsByType(type) {
    try {
      ...
      const countMethodName =
        type === 'issuer' ? 'issuerProposalCount' : 'verifierProposalCount';
      const fetchMethodName =
        type === 'issuer' ? 'getIssuerProposal' : 'getVerifierProposal';
  ...
  async function handleLoadAllProposals() {
    ...
    try {
      ...
      toast.success(
        `Loaded ${issuerProposals.length} issuer + ${verifierProposals.length} verifier proposal(s)`
      );
    } catch (error) {
      console.error('Load all proposals error:', error);
      toast.error(
        error?.shortMessage || error?.message || 'Unable to load proposals'
      );
      onTxStatus?.({
        state: 'error',
  ...
  const verifierProposals = filteredProposals.filter(
    (p) => p.type === 'verifier'
  );
  const showIssuerLane =
    proposalLaneFilter === 'all' || proposalLaneFilter === 'issuer';
  const showVerifierLane =
    proposalLaneFilter === 'all' || proposalLaneFilter === 'verifier';
          ...
          <div className="cyber-card">
            ...
            >
              ...
              <div style={{ flexGrow: 1 }}>
                ...
                <p className="cyber-subheading" style={{ marginBottom: 0 }}>
                  Add, search, and manage user identity records securely on the
                  VeriChain network.
                </p>
              </div>
              ...
              <div className="form-section slide-up">
                ...
                <div className="card-actions" style={{ marginTop: '2rem' }}>
                  <button
                    ...
                    onClick={() => wrapAction(handleAddKYC)}
            ...
            {kycSubTab === 'lookup' && (
              <div className="form-section slide-up">
                <div className="form-group">
                  ...
                  >
                    Changes here sync with the registration form.
                  </small>
                </div>
                ...
                {kycRecord && (
                  <div
                    className="data-display slide-up"
                    style={{
                      marginTop: '2rem',
            ...
            >
              ...
              <div style={{ flexGrow: 1 }}>
                ...
                <p className="cyber-subheading">
                  Identify and flag suspicious on-chain behavior associated with
                  KYC records.
                </p>
              </div>
            ...
            {fraudSubTab === 'check' && (
              <div className="form-section slide-up">
                <div className="form-group">
                  ...
                  >
                    Changes here sync with the reporting form.
                  </small>
                </div>
            ...
            {fraudSubTab === 'report' && (
              <div className="form-section slide-up">
                ...
                <div className="card-actions" style={{ marginTop: '2rem' }}>
                  <button
                    className="cyber-button"
                    onClick={() => wrapAction(handleReportFraud)}
                    style={{
                      width: '100%',
                      justifyContent: 'center',
                      background:
                        'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)',
                      boxShadow: '0 8px 20px -5px rgba(244, 63, 94, 0.4)',
                    }}
                  >
                    <span className="btn-icon">
                      <AlertTriangle size={16} />
          ...
          <div className="cyber-card">
            ...
            >
              ...
              <div style={{ flexGrow: 1 }}>
                ...
                <p className="cyber-subheading" style={{ marginBottom: 0 }}>
                  Participate directly in network governance by submitting,
              ...
              <div className="form-section slide-up">
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setProposal({ ...proposal, target: e.target.value })
                      }
                      placeholder="0x..."
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProposal({ ...proposal, evidenceURI: e.target.value })
                    }
                ...
                <div className="card-actions">
                  <button
                    className="cyber-button"
                    onClick={() => wrapAction(handleCreateProposal)}
                  >
                    <span className="btn-icon">
                      <FileText size={16} />
                    </span>
            ...
            {govSubTab === 'vote' && (
              <div className="form-section slide-up">
                ...
                <div className="card-actions">
                  <button
                    className="cyber-button"
                    onClick={() => wrapAction(handleViewProposal)}
                  >
                    <span className="btn-icon">
                      <Search size={16} />
                    </span>
                  ...
                  <button
                    className="cyber-button"
                    onClick={() => wrapAction(handleCancelProposal)}
                  >
                    <span className="btn-icon">
                      <AlertTriangle size={16} />
                    </span>
            ...
            {govSubTab === 'list' && (
              <div className="form-section slide-up">
                ...
                >
                  ...
                  >
                    All proposals
                    <span className="proposal-lane-tab__hint">
                      Issuer + Verifier
                    </span>
                  </button>
                  <button
                  ...
                  >
                    Issuer only
                    <span className="proposal-lane-tab__hint">
                      {issuerProposals.length} in view
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    Verifier only
                    <span className="proposal-lane-tab__hint">
                      {verifierProposals.length} in view
                    </span>
                  </button>
                </div>
                ...
                {filteredProposals.length === 0 ? (
                  <div className="empty-state">
                    ...
                    <p className="empty-text">
                      No proposals found for selected status
                    </p>
                    <p className="empty-hint">
                      Click "Load Proposals" and change status filter
                    </p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    {showIssuerLane && (
                      <section
                        className="proposals-section proposals-section--issuer proposals-section--standalone"
                        aria-labelledby="issuer-proposals-heading"
                      >
                        <header className="proposals-section__header proposals-section__header--issuer">
                          <div className="proposals-section__title-wrap">
                            <span className="proposals-section__eyebrow">
                              Section A
                            </span>
                            <h3
                              id="issuer-proposals-heading"
                              className="proposals-section__title"
                            >
                              Issuer proposals
                            </h3>
                            <p className="proposals-section__desc">
                              On-chain votes to add or change issuer roles
                            </p>
                          </div>
                          <span className="proposals-section__count proposals-section__count--issuer">
                            {issuerProposals.length}
                          </span>
                        </header>
                        {issuerProposals.length === 0 ? (
                          <p className="proposals-section__empty">
                            No issuer proposals for this status filter.
                          </p>
                        ) : (
                    ...
                    {showVerifierLane && (
                      <section
                        className="proposals-section proposals-section--verifier proposals-section--standalone"
                        aria-labelledby="verifier-proposals-heading"
                      >
                        <header className="proposals-section__header proposals-section__header--verifier">
                          <div className="proposals-section__title-wrap">
                            ...
                            <h3
                              id="verifier-proposals-heading"
                              className="proposals-section__title"
                            >
                              Verifier proposals
                            </h3>
                            <p className="proposals-section__desc">
                              On-chain votes to add or change verifier roles
                            </p>
                          </div>
                          <span className="proposals-section__count proposals-section__count--verifier">
                            {verifierProposals.length}
                          </span>
                        </header>
                        {verifierProposals.length === 0 ? (
                          <p className="proposals-section__empty">
                            No verifier proposals for this status filter.
                          </p>
                        ) : (
                          <div className="proposals-grid proposals-grid--verifier">
                            {verifierProposals.map((item) => (
                              <ProposalCard
                                key={`verifier-${item.id}`}
                                item={item}
                              />
                            ))}
                          </div>
...
function ProposalCard({ item }) {
  ...
  return (
    <article className={`proposal-card proposal-card--${variant}`}>
      <div
        className={`proposal-card__accent proposal-card__accent--${variant}`}
        aria-hidden
      />
      <div className="proposal-card__header">
        <div className="proposal-card__id-block">
          <span
            className={`proposal-card__pill proposal-card__pill--${variant}`}
          >
            {label}
          </span>
        ...
        <span
          className={`status-badge status-${item.statusLabel.toLowerCase()}`}
        >
          {item.statusLabel}
        </span>
      </div>
      ...
      <div className="proposal-card__body">
        <dl className="proposal-card__meta">
          <div className="proposal-card__meta-row">
            <dt>Target</dt>
            <dd className="proposal-card__mono">
              {item.proposedTarget || '—'}
            </dd>
          </div>
          <div className="proposal-card__meta-row">
          ...
          <div className="proposal-card__meta-row proposal-card__meta-row--full">
            <dt>Justification</dt>
            <dd className="field-value description">
              {item.justification || '—'}
            </dd>
          </div>
          {item.evidenceURI ? (
            <div className="proposal-card__meta-row proposal-card__meta-row--full">
              ...
              <dd>
                <a
                  ...
                  className={`field-link proposal-card__link--${variant}`}
                >
                  Open link
                </a>
              </dd>
        ...
        <div className={`proposal-stats proposal-stats--${variant}`}>
          <div className="stat-item">
            ...
            <span className="stat-value approve">
              {String(item.approvals ?? 0)}
            </span>
          </div>
          <div className="stat-item">
            <span className="stat-label">Rejections</span>
            <span className="stat-value reject">
              {String(item.rejections ?? 0)}
            </span>
          </div>
          <div className="stat-item">
            <span className="stat-label">Deadline</span>
            <span className="stat-value proposal-card__mono">
        ...
        <details className="proposal-details-toggle">
          ...
          <pre className="data-block details-content">
            {toDisplayJson(item)}
          </pre>
        </details>
      </div>
